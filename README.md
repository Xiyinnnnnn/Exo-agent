...
|------|--------|---------|------|------|----------|
| `fast` | 150 | 4 | 12 | ✗ | ~3.4M (日配额10%) |
| `standard` | 600 | 6 | 20 | ✓ | ~6.9M (日配额20%) |
...
**长对话压缩:**
- 智能压缩：保留最近1轮对话，在其下方追加历史摘要
- 压缩积极度可调，最大944K
- 大文件回退式精炼，N-3轮对上下文减量，保证任务稳定性
...
### 13. 上下文工程与缓存命中率优化
通过系统级上下文复用缓存机制，系统冷启动时缓存命中率达 **92%**，热机多对话复杂使用环境下稳定在 **97.98%** 浮动。针对极限代码审计等单场景任务，极限命中率可达 **99.21%**（浮动值，不能稳定触发 99.87%）。整体Agent实际输出率仅占 **0.39%**，意味着99.61%的响应从缓存直接命中，大幅削减API调用成本，同时保持上下文的一致性和完整性。
...
| 本系统·冷启动 | 92% | 0.39% | ~$0.0148 | **~8.5x** |
| 本系统·稳态多对话 | 97.98% | 0.39% | ~$0.00664 | **~19.0x** |
| 本系统·极限代码审计 | 99.21% | 0.39% | ~$0.00496 | **~25.4x** |
